...
export default async function handler(req, res) {
  ...
  if (pathname === '/sitemap-chungcheong') {
    ...
    const hubDesc = "대전광역시, 세종특별자치시, 충청북도(청주·충주) 및 충청남도(천안·아산·서산·당진) 주요 구·동 단위의 탄성코트 전문 시공 서비스 페이지 안내 목록입니다.";
    ...
    // Grouping for Chungcheong
    const chungcheongGroups = {
      '대전권': { label: '대전광역시', districts: {} },
      '세종권': { label: '세종특별자치시', districts: {} },
      '충북권': { label: '충청북도 (청주·충주)', districts: {} },
      '충남권': { label: '충청남도 (천안·아산·서산·당진)', districts: {} }
    };
    ...
    activeList.forEach(r => {
      ...
      if (r.metro === '대전') {
        ...
        distKey = (r.groupName && r.groupName !== '청주시' && r.groupName !== '충주시') ? r.groupName : (r.groupName || '시 단위');
      } else if (r.metro === '충남') {
    ...
    seoContent += '<p style="color: #666; margin-bottom: 30px;">대전광역시, 세종특별자치시, 충청북도(청주·충주) 및 충청남도(천안·아산·서산·당진) 주요 구·동 단위의 탄성코트 전문 시공 서비스 안내 목록입니다.</p>';
